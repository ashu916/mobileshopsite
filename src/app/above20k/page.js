...
async function getData(){
  try {
  const res =  await fetch('http://localhost:3000/api/above20k',{cache:'no-store'})
...
<div className="w-3/4">
{item.VariantAndPrices.map((variant)=>{return(<p className="text-red-500 text-sm">{variant}</p>)})}
</div>
